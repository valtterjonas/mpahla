...
if ($_SESSION['loggedin']) {
    ...
    $reservas = select("r_reserva,u_utilizador,c_caixa",
        ...
        "WHERE c_caixa.c_id = r_reserva.c_id AND u_utilizador.u_id = r_reserva.u_id AND r_reserva.u_id = '$cliente_id' ORDER BY r_reserva.r_id DESC");
    ...
    if($reservas){
        ...
        for($i = 0; $i < count($reservas); $i++){
            ...
            $preco = $reservas[$i]["c_preco"];
            $preco = number_format($preco,2,'.',',')." MT";
            $qtd = $reservas[$i]["r_quantidade"];
            $estado = $reservas[$i]["r_estado"];

            $span_pendente = "primary";
            ...
            }else if(strcmp($estado,"pendente") == 0){
            ...
            $resposta.='
                                <tr>
									...
									<td>
										<a class="main-btn " href="reserva-page.html?token='.$r_id.'"><i class="fa fa-search-plus"></i></a>
									</td>
								</tr>
